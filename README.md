...
- **状态五态**：`待确认 / 已确认待修改 / 已修改 / 忽略 / 延后再改`。
- **批量修改状态**：单个「批量修改状态」按钮 + 下拉菜单任选目标状态，**任意状态 → 任意目标状态**（无硬性状态机限制），**仅项目创建者可操作**，逐条校验并报告跳过原因。
- **评论编辑 / 删除**：编辑内容；删除为软删（创建者跨状态可操作、作者限可编辑态）。
- **评论导出 zip（仅创建者）**：`scope=all`（全量）或 `scope=confirmed`（已确认待修改，标准交付范围）；导出包 `manifest.json + comments/ + shots/`，与项目目录同构。
- **评论抽屉（前端）**：右侧三栏布局（原型 / PRD / 评论，均可拖拽调宽）；标题栏显示评论与已选数量；宿主筛选 + 状态（五态）筛选；单个「批量修改状态」按钮；按页面分组、两级折叠默认全部展开；无锚点段落按“页面 / 无锚点段落”分组展示。
...
- **轻量迁移**：`init_tables()` 幂等建表 + `_migrate()` 用 `ALTER TABLE` 补/删列（`users.disabled`、`comments.priority/scope`），`seed_admin()` 幂等种子超管。
- **权限模型**：
  - 登录：`session["uid"]` + `before_request` 全局拦截（失效即 401 强制登出）。
  - 超管：`is_admin`，可访问 `/api/users`、**删除任意项目**。
  - 创建者：上传 / 导出 / 可评论开关 / 批量改状态 / 管理任意评论，均校验 `creator_id`。
...
- **单元 + 契约测试**：`server/tests/test_*.py`，覆盖 seed 超管、停用拦截、用户 CRUD 权限、上传安全、评论五态批量流转、对账、导出等。
